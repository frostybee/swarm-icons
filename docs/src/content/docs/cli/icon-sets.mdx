...
php vendor/bin/swarm-icons json:download --list
```

You can specify a different destination with `--dest`:

```bash
php vendor/bin/swarm-icons json:download mdi --dest=resources/json
```
...
Every download is recorded in a `swarm-icons.json` manifest at your project root, along with the npm version of each set. Running `json:download` with no arguments re-downloads everything in the manifest (useful for CI/CD or restoring after a fresh clone).
...
php vendor/bin/swarm-icons icon:search custom home --path=/path/to/svgs

# Search a downloaded JSON collection
php vendor/bin/swarm-icons icon:search mdi account --json=resources/json/mdi.json
```
